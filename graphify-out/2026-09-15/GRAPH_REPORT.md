...
Cohesion: 0.12
Nodes (20): _agent_factory_for(), factory(), Return an agent factory for the worker type, or None (placeholder loop)., Result of a single ``opencode run`` invocation., RunResult, OpenCodeWorkerAgent, The OpenCode worker's brain. Drives a milestone-based workflow: for each step…, Queue guidance from the parent (user / Jarvis) for the next milestone. (+12 more)

### Community 2 - "resolve_path"
Cohesion: 0.11
Nodes (17): Path, Resolve a user‑provided path string to an absolute Path. - Expands ``~`` to the…, resolve_path(), ensure_is_dir(), Any, Any, _file_hash(), Any (+9 more)

### Community 3 - "get_client"
Cohesion: 0.07
Nodes (23): get_client(), Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_group_action(), Any, Any, Any, Summarize a file or folder and send the summary as a WhatsApp message. (+15 more)

### Community 4 - "registry/__init__.py"
Cohesion: 0.09
Nodes (28): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, AppendFileTool, Any (+20 more)
...
Cohesion: 0.14
Nodes (11): BaseModel, Ensure default allowed_tools is never None., Deserialize from JSON received from the parent., The task contract defines what a worker is expected to do, with strict…, TaskContract, _contract(), asyncio, The worker must not be able to call tools outside allowed_tools. (+3 more)

### Community 13 - "workers.py"
Cohesion: 0.13
Nodes (21): cancel_worker(), _disk_sessions(), fork_worker(), ForkRequest, get_worker(), get_worker_artifact_file(), get_worker_result(), intervene_worker() (+13 more)
...
### Community 29 - "ListRecentEmailsSkill"
Cohesion: 0.12
...
### Community 31 - "test_opencode_worker.py"
Cohesion: 0.17
Nodes (10): setup_logging(), Non-interactive OpenCode CLI client. Wraps ``opencode run`` as an async…, get_opencode_binary(), get_opencode_server_url(), Configuration for the OpenCode CLI Worker. Resolves the ``opencode`` binary…, Return the path to the ``opencode`` CLI binary, or None if not found., Return the URL of a running headless OpenCode server (for ``--attach``)., OpenCode Worker Agent — Jarvis's coding worker brain. Implements the agent… (+2 more)

### Community 32 - "run_opencode"
Cohesion: 0.16
Nodes (12): _build_args(), _extract_session_id(), Any, Try to extract the OpenCode session ID from events or raw output., Execute ``opencode run`` and collect structured results. Parameters ----------…, Build the ``opencode run`` argument list., Execute opencode in a thread worker (optionally in a visible Windows terminal…, run_opencode() (+4 more)
...
### Community 48 - "Milestone"
Cohesion: 0.16
Nodes (12): build_prompt(), Milestone, MilestonePhase, plan_milestones(), Enum, str, Milestone Planner & Prompt Builder for Jarvis → OpenCode orchestration. Jarvis…, Build the full prompt string sent to ``opencode run``. Parameters ----------… (+4 more)
...
### Community 52 - "test_mutation_tools.py"
Cohesion: 0.60
...
### Community 60 - "TestConfig"
Cohesion: 0.19
Nodes (7): get_auto_approve(), get_milestone_timeout(), get_opencode_model(), Return the OpenCode model identifier (provider/model format), or None to use…, Return whether to run OpenCode with ``--auto`` (auto-approve permissions)., Maximum seconds to wait for a single milestone execution., TestConfig
...
### Community 95 - "execution_engine.py"
Cohesion: 0.32
...
### Community 100 - "organize_downloads_pipeline.py"
Cohesion: 0.33
...
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `registry/__init__.py`, `organize_downloads_pipeline.py`, `skills.py`, `tools.py`, `ToolRegistry`, `TaskContract`, `test_whatsapp.py`, `test_search_content.py`, `WorkerEngine`, `agent.py`, `test_mutation_tools.py`, `test_tools_and_routes.py`, `test_file_tools_v2.py`, `main.py`, `ListRecentEmailsSkill`, `execution_engine.py`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `WorkerEngine` connect `WorkerEngine` to `OpenCodeWorkerAgent`, `ToolRegistry`, `worker_events`, `TaskContract`, `workers.py`, `WorkerSession`, `ExecutionEngine`, `test_agent_fork.py`, `EventBus`, `test_opencode_worker.py`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `TaskContract` connect `TaskContract` to `OpenCodeWorkerAgent`, `workers.py`, `WorkerSession`, `WorkerEngine`, `agent.py`, `test_agent_fork.py`, `.model_dump_json`, `test_opencode_worker.py`?**
