...
Cohesion: 0.08
Nodes (30): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, OrganizeDownloadsSkill, Organize files in a download directory into subfolders by file extension.… (+22 more)
...
Cohesion: 0.08
Nodes (23): get_client(), Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_group_action(), PhotoBackupPipeline, Any, Download photos from a WhatsApp chat and store them in YYYY/MM folders., Any (+15 more)
...
Cohesion: 0.35
Nodes (10): ExecutionEngine, Core engine that validates input, executes a tool, and returns a ToolResult.…, asyncio, fixture, setup_registry(), test_send_email_happy_path_no_creds(), test_send_email_missing_attachment_fails_fast(), test_send_email_missing_params() (+2 more)
...
Cohesion: 0.07
Nodes (25): event_generator(), get, stream_events(), get_health(), HealthResponse, BaseModel, get, Settings (+17 more)
...
### Community 78 - "Session: Worker / Orchestrator Architecture — Steps 0–1 (2026-09-12)"
Cohesion: 0.33
...
Cohesion: 0.15
Nodes (20): ArchiveOldFilesPipeline, Zip files that have not been modified in the last N months, then remove the…, BackupDocumentsPipeline, One-way incremental backup: copy files that are new or newer than the backup…, DedupeFolderPipeline, _file_hash(), Path, Find duplicate files (by size + SHA-256) and move extras into a review folder. (+12 more)
...
### Community 100 - "1. Module skeleton"
Cohesion: 0.40
...
### Community 123 - "Session: Gmail Tools, OAuth & Agent Email Routing (2026-09-12)"
Cohesion: 0.40
...
Cohesion: 0.10
Nodes (21): ensure_exists(), ensure_is_dir(), is_protected(), Path, Return True if the path is within a protected location., Validate that a path is safe to operate on. Raises ``ValueError`` if the path…, validate_path(), Any (+13 more)
...
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `registry/__init__.py`, `ListRecentEmailsSkill`, `ToolRegistry`, `TaskContract`, `execution_engine.py`, `test_search_content.py`, `test_whatsapp.py`, `test_tools_and_routes.py`, `WorkerEngine`, `test_mutation_tools.py`, `agent.py`, `JarvisException`, `OrganizeDownloadsPipeline`, `test_file_tools_v2.py`, `skills.py`, `tools.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `ToolRegistry` connect `ToolRegistry` to `registry/__init__.py`, `TaskContract`, `execution_engine.py`, `ExecutionEngine`, `OpenCodeAdapter`, `.__init__`, `JarvisBrainManager`, `WorkerEngine`, `ScopedToolRegistry`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `JarvisBrainManager` connect `JarvisBrainManager` to `ToolRegistry`, `.__init__`, `.analyze_prompt`?**
