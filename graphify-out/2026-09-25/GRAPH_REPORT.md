# Graph Report - AI-Automation  (2026-09-25)

## Corpus Check
- 261 files · ~169,042 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 58 file(s) not represented in the graph (top: .vrma 41, (none) 5, .bat 5)

## Summary
- 2397 nodes · 4525 edges · 192 communities (141 shown, 34 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 343 edges (avg confidence: 0.93)
...
## Communities (192 total, 34 thin omitted)
...
Cohesion: 0.13
Nodes (28): Summarize a file or folder and send the summary as a WhatsApp message., SendReportSkill, asyncio, Tests for the WhatsApp module using a fake sidecar client., test_daily_digest_pipeline(), test_download_media_tool(), test_downloads_notifier_pipeline(), test_get_messages_tool() (+20 more)

### Community 2 - "registry/__init__.py"
Cohesion: 0.10
Nodes (27): format_file_size(), get_drive_creds(), get_drive_service(), get_token_path(), is_mock_mode(), Credentials, Path, Find drive_token.json or fallback token.json in project paths. (+19 more)

### Community 3 - "workers.py"
Cohesion: 0.06
Nodes (54): _agent_factory_for(), approve_worker_plan(), ApprovePlanRequest, cancel_worker(), clear_all_workers(), _disk_sessions(), fork_worker(), ForkRequest (+46 more)

### Community 4 - "BaseTool"
Cohesion: 0.09
Nodes (29): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, AppendFileTool, Any (+21 more)

### Community 5 - "VRMAvatar.js"
Cohesion: 0.15
Nodes (5): AvatarBreathing, AvatarGaze, AvatarIdle, VRMAvatar, three

### Community 6 - "server.js"
Cohesion: 0.07
Nodes (31): dependencies, express, qrcode-terminal, venom-bot, description, main, name, scripts (+23 more)

### Community 7 - "._execute_hook"
Cohesion: 0.22
Nodes (6): Any, Handle worker completion and trigger all registered hooks., Select the most relevant non-test business artifact for this specific hook., Interpolate placeholders and infer missing file/attachment arguments., Self-healing check: if worker is already finished on disk, trigger pending…, Register a follow-up action to execute as soon as target_session_id completes.
...
### Community 12 - "test_session_handover.py"
Cohesion: 0.14
Nodes (19): isolated_sessions(), asyncio, fixture, Path, Test launch_worker automatically detects existing handover in workspace., Test that graphify knowledge graph is signaled in handover section and contract., Test that _fork_task respects explicit fs_scope or extracts directory from…, Test the GET /workers/{session_id}/handover API endpoint. (+11 more)

### Community 13 - "AvatarCanvas.js"
Cohesion: 0.15
Nodes (5): AvatarCanvas(), loadModel(), AvatarScene, SemanticCueEngine, AvatarCanvas

### Community 14 - "AntigravityWorkerAgent"
Cohesion: 0.11
Nodes (17): factory(), Result of a single `agy run` invocation., RunResult, AntigravityWorkerAgent, Any, Record step result into agent history., Queue parent/manager guidance for the next turn., Autonomous agent driver powered by Antigravity CLI (`agy`) with 3-Job… (+9 more)

### Community 15 - "JarvisEvent"
Cohesion: 0.09
Nodes (19): AbstractEventLoop, GlobalEventBus, Queue, Central Async Event Spine for Jarvis. Supports: - Multi-subscriber SSE queue…, Explicitly set or update the main asyncio loop for cross-thread calls., Return a snapshot of recently published events in chronological order., Subscribe a new asyncio.Queue to receive live streaming events., Remove an asyncio.Queue from active SSE subscribers. (+11 more)
...
Cohesion: 0.09
Nodes (20): JarvisBrainManager, Any, Path, Ensure JARVIS_MEMORY.md exists on disk., Read the full content of JARVIS_MEMORY.md., Append an entry to the scratchpad section in JARVIS_MEMORY.md. Deduplicates…, Record activity timestamp to reset idle timer., Evaluate instant conversational intents (<5ms response time). (+12 more)
...
Cohesion: 0.06
Nodes (39): Path, Resolve a user-provided path string to an absolute Path. - Rejects empty…, resolve_path(), move_to_trash(), Path, Recoverable-delete support: moves items into a backend-local trash folder., Move a file or folder into the trash directory and return its new path. The…, ensure_exists() (+31 more)

### Community 25 - "TaskContract"
Cohesion: 0.10
Nodes (21): BaseModel, Ensure default allowed_tools is never None., Serialize to JSON for transport (parent ↔ worker session init)., The task contract defines what a worker is expected to do, with strict…, TaskContract, Create the on‑disk worker session from a contract., ActiveStreamGuardian, json_dump_safe() (+13 more)

### Community 26 - "test_agent_fork.py"
Cohesion: 0.23
Nodes (14): _execute_plan(), _fork_task(), Execute plan steps sequentially and return collected results., Fork a background worker session via the workers module and record a tracked…, get_engine(), Used by the agent route to look up engines after delegation., isolated_sessions(), asyncio (+6 more)
...
Cohesion: 0.07
Nodes (11): Path, Append a single JSON line to events.jsonl., Read structured session handover brief., Write structured session handover brief., Create the session directory and write the initial contract. Returns self for…, Manages a single worker's on-disk session state. On fork, creates the following…, WorkerSession, client() (+3 more)
...
Cohesion: 0.11
Nodes (18): 1. Module skeleton, 2. Register the new components, 2b. Make the tool available to the AGENT (essential!), 3.1 Tools endpoint, 3.2 Pipelines endpoint, 3. Expose the module via the API, 4. Optional: Add helper imports, 5. Quick checklist (+10 more)

### Community 36 - "test_file_tools_v2.py"
Cohesion: 0.10
Nodes (38): BulkRenameTool, CopyTool, CreateFileTool, CreateFolderTool, DeleteFileTool, DeleteFolderTool, ExtractTool, MoveTool (+30 more)
...
Cohesion: 0.12
Nodes (8): ArtifactsPanel(), EVENT_STYLE, formatBytes(), formatEventsToTerminal(), HandoverAndGraphPanel(), STATUS_STYLE, WorkerPage(), poll()
...
Cohesion: 0.19
Nodes (9): Reactive Event-Driven Task Orchestrator. Allows Jarvis to chain dependent tasks…, TaskOrchestrator, EventType, Enum, str, ReactiveHook, setup_logging(), test_events_recent_endpoint() (+1 more)

### Community 43 - "FakeWhatsAppClient"
Cohesion: 0.12
...
Cohesion: 0.10
Nodes (15): emit(), emit_to_queues(), EventBus, Any, Queue, Helper that gives the worker engine a clean ``await bus.xxx`` API., Subscribe a new asyncio.Queue to receive live streaming events for session_id., Unsubscribe and remove an asyncio.Queue from session_id's active subscribers. (+7 more)
...
Cohesion: 0.13
Nodes (18): client(), fixture, test_unknown_worker_still_404s(), client(), fixture, test_events_feed_and_digest_routes(), asyncio, Regression tests for tool fixes and API routes. (+10 more)
...
### Community 49 - "manage_chat.py"
Cohesion: 0.15
Nodes (7): Connection states of the WhatsApp sidecar and human-readable descriptions., Validation helpers for WhatsApp tools., validate_chat_action(), GetStatusTool, Any, ManageChatTool, Any

### Community 50 - "DriveInboundListener"
Cohesion: 0.28
Nodes (4): DriveInboundListener, Any, Persist drive update to SQLite service_events table., Continuous background listener that monitors Google Drive for recently modified…
...
Cohesion: 0.08
Nodes (18): AgentFactory, # NOTE: params must match each tool's ``input_schema`` exactly — the plan, Registry that holds tool classes keyed by their name. Usage: registry =…, ToolRegistry, Any, Return the tool instance if it is in the allowed set, else raise., Return only the allowed tools (name → tool instance)., Return the set of permitted tool names. (+10 more)
...
Cohesion: 0.11
Nodes (29): Synthesize a complete Master Task Specification prompt for autonomous workers., build_execution_prompt(), build_master_task_prompt(), build_planning_prompt(), build_prompt(), build_testing_prompt(), _format_handover_section(), is_simple_task() (+21 more)
...
Cohesion: 0.20
Nodes (12): PowerControls(), handleCancelShutdown(), handleLock(), handleShutdownConfirm(), apiFetch(), getHeaders(), ForkForm(), submit() (+4 more)
...
Cohesion: 0.13
Nodes (23): get_activity_feed(), get_feed_unread_counts(), get_recent_events(), get_seven_day_digests(), get_task_hooks(), HookRegistrationRequest, ManualEventRequest, publish_manual_event() (+15 more)

### Community 63 - "app/page.js"
Cohesion: 0.29
...
### Community 69 - "execution_engine.py"
Cohesion: 0.24
...
### Community 72 - "Home"
Cohesion: 0.27
Nodes (9): generateId(), Home(), callAgent(), checkHealth(), confirmPlan(), openOpenCodeTerminal(), sendPrompt(), triggerAvatarSpeech() (+1 more)
...
Cohesion: 0.22
Nodes (6): OrganizeDownloadsPipeline, Any, Pipeline that runs the OrganizeDownloadsSkill. It demonstrates how higher‑level…, OrganizeDownloadsSkill, Path, Organize files in a download directory into subfolders by file extension.…
...
Cohesion: 0.26
Nodes (12): get_orchestrator(), lifespan(), _periodic_memory_maintenance(), FastAPI, Periodic background task to aggregate daily digests and prune expired 7-day…, prewarm_vector_embedder(), Pre-warm the ONNX embedder model in a background thread at startup., get_drive_listener() (+4 more)
...
### Community 97 - "brain.py"
Cohesion: 0.10
Nodes (23): get_brain_manager(), Jarvis Central Brain Manager powered by Antigravity CLI (`agy.exe`). Features:…, Return the global JarvisBrainManager instance., Antigravity module for Jarvis core brain and memory., _build_args(), _extract_session_id(), Any, Non-interactive Antigravity CLI client. Wraps `agy run` (or configured CLI) as… (+15 more)
...
Cohesion: 0.19
Nodes (18): ArchiveOldFilesPipeline, Zip files that have not been modified in the last N months, then remove the…, BackupDocumentsPipeline, One-way incremental backup: copy files that are new or newer than the backup…, DedupeFolderPipeline, Find duplicate files (by size + SHA-256) and move extras into a review folder., OrganizePhotosPipeline, Sort image files into YYYY/MM subfolders based on their modified date. (+10 more)

### Community 117 - "get_client"
Cohesion: 0.06
Nodes (26): get_client(), Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_group_action(), Any, Any, Collect all unread WhatsApp chats and their recent messages into one digest., UnreadDigestSkill (+18 more)
...
Cohesion: 0.20
Nodes (11): get_event_bus(), Attach listener to the global event bus., MockWhatsAppTool, asyncio, test_task_orchestrator_auto_discovers_workspace_file(), test_task_orchestrator_chains_worker_completion(), test_task_orchestrator_interpolates_email_attachments(), __init__() (+3 more)
...
### Community 166 - "test_antigravity_brain.py"
Cohesion: 0.28
...
### Community 176 - "Chat.js"
Cohesion: 0.29
Nodes (3): BotMessage(), UserBubble(), ToolOutputViewer()

### Community 177 - "list_pipelines"
Cohesion: 0.29
...
### Community 179 - "Session: Fork UX — agent `fork` tool, manual fork form, chat persistence (2026-09-12)"
Cohesion: 0.33
...
### Community 180 - "Session: Worker / Orchestrator Architecture — Steps 0–1 (2026-09-12)"
Cohesion: 0.33
...
### Community 185 - "DailyDigestPipeline"
Cohesion: 0.40
Nodes (3): DailyDigestPipeline, Any, Run the unread digest and send it to the given chat (run on a schedule or on…

### Community 186 - "7. Write unit tests"
Cohesion: 0.50
Nodes (4): 7. Write unit tests, Running the tests, Testing a pipeline, Testing a tool
...
- **Why does `ToolRegistry` connect `ToolRegistry` to `brain.py`, `ExecutionEngine`, `registry/__init__.py`, `BaseTool`, `execution_engine.py`, `test_file_tools_v2.py`, `test_tools_and_routes.py`, `WorkerEngine`, `JarvisBrainManager`, `test_search_content.py`, `OpenCodeAdapter`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `BaseTool` connect `BaseTool` to `test_whatsapp.py`, `ExecutionEngine`, `registry/__init__.py`, `test_file_tools_v2.py`, `execution_engine.py`, `verify_exists`, `get_event_bus`, `ListRecentEmailsSkill`, `.execute`, `OrganizeDownloadsSkill`, `manage_chat.py`, `ToolRegistry`, `get_client`, `resolve_path`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `agent.py`, `test_whatsapp.py`, `main.py`, `BaseTool`, `execution_engine.py`, `test_file_tools_v2.py`, `._execute_hook`, `EventType`, `ListRecentEmailsSkill`, `FakeWhatsAppClient`, `OrganizeDownloadsSkill`, `skills.py`, `tools.py`, `test_tools_and_routes.py`, `WorkerEngine`, `ToolRegistry`, `test_search_content.py`?**
