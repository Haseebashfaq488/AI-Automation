...
Cohesion: 0.17
Nodes (4): AvatarBreathing, AvatarIdle, VRMAvatar, three
...
Cohesion: 0.11
Nodes (18): Execute `agy` via stream-json stdin/stdout and collect structured results., Result of a single `agy run` invocation., run_antigravity_cli(), RunResult, AntigravityWorkerAgent, Any, Record step result into agent history., Queue parent/manager guidance for the next turn. (+10 more)
...
Cohesion: 0.06
Nodes (45): Path, Resolve a user-provided path string to an absolute Path. - Rejects empty…, resolve_path(), ensure_exists(), ensure_is_dir(), is_protected(), Path, Return True if the path is within a protected location. (+37 more)
...
Cohesion: 0.09
Nodes (38): ArchiveTool, BulkRenameTool, CopyTool, CreateFileTool, CreateFolderTool, DeleteFolderTool, ExistsTool, Any (+30 more)
...
Cohesion: 0.14
Nodes (14): _extract_session_id(), PersistentAgyDaemon, Any, Non-interactive Antigravity CLI client. Wraps `agy run` (or configured CLI) as…, Maintains a persistent, warm background Antigravity CLI process. Uses `agy…, Start or verify the persistent background daemon process., Send a turn over the persistent stdin/stdout pipe with fallback., Gracefully stop the persistent daemon. (+6 more)
...
### Community 50 - "whatsapp/helpers/validation.py"
Cohesion: 0.25
Nodes (5): Validation helpers for WhatsApp tools., validate_chat_action(), validate_group_action(), Any, Any
...
Cohesion: 0.12
Nodes (23): AvatarStudioPage(), handleIntensityChange(), pollVolume(), runChoreography(), stopMotion(), triggerEmotion(), BASIC_VRM_EMOTIONS, CHARM_EMOTIONS (+15 more)
...
### Community 85 - "OpenCodeWorkerAgent"
Cohesion: 0.15
...
### Community 94 - "app/page.js"
Cohesion: 0.16
Nodes (13): BotMessage(), UserBubble(), getTTSVoice(), generateId(), Home(), callAgent(), checkHealth(), confirmPlan() (+5 more)
...
Cohesion: 0.11
Nodes (14): Jarvis Central Brain Manager powered by Antigravity CLI (`agy.exe`). Features:…, Antigravity module for Jarvis core brain and memory., _build_args(), Build the argument list for official Antigravity CLI (`agy.exe`)., get_antigravity_model(), get_auto_approve(), get_milestone_timeout(), get_visible_console() (+6 more)
...
Cohesion: 0.06
Nodes (56): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, Search Google Drive files by keyword, full-text content, and file type.…, SearchDriveSkill (+48 more)
...
### Community 176 - "_agent_factory_for"
Cohesion: 0.15
...
### Community 179 - "UnreadDigestSkill"
Cohesion: 0.28
...
### Community 203 - "DownloadsNotifierPipeline"
Cohesion: 0.67
...
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `agent.py`, `test_whatsapp.py`, `OrganizeDownloadsPipeline`, `test_file_tools_v2.py`, `ListRecentEmailsSkill`, `TaskOrchestrator`, `execution_engine.py`, `EventType`, `OrganizeDownloadsSkill`, `engine`, `skills.py`, `tools.py`, `main.py`, `WorkerEngine`, `test_tools_and_routes.py`, `test_search_content.py`, `BaseTool`, `ToolRegistry`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `WorkerSession` connect `WorkerSession` to `test_opencode_worker.py`, `TaskContract`, `workers.py`, `TaskOrchestrator`, `EventType`, `worker_events`, `post`, `_agent_factory_for`, `main.py`, `WorkerEngine`, `_contract`, `OpenCodeWorkerAgent`, `ToolRegistry`, `test_agent_fork.py`, `test_session_handover.py`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `Repository` connect `Repository` to `agent.py`, `GmailInboundListener`, `EventType`, `ServiceMemoryManager`, `._persist_to_db`, `create_task`, `test_agent_fork.py`, `service_memory.py`, `DriveInboundListener`?**
