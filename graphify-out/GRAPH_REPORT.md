...
## Corpus Check
- 270 files · ~180,533 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 63 file(s) not represented in the graph (top: .vrma 43, (none) 5, .bat 5)

## Summary
- 2486 nodes · 4677 edges · 191 communities (143 shown, 31 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 348 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b1222edf`
...
- worker_events
- dependencies
- test_plan_review_and_skip_testing.py
- Worker Statuses
- jarvis-backend
...
Cohesion: 0.13
Nodes (10): move_to_trash(), Path, Recoverable-delete support: moves items into a backend-local trash folder., Move a file or folder into the trash directory and return its new path. The…, ensure_is_file(), DeleteFileTool, Any, Any (+2 more)
...
Cohesion: 0.12
Nodes (16): devDependencies, eslint, eslint-config-next, tailwindcss, @tailwindcss/postcss, name, private, version (+8 more)

### Community 21 - "JarvisBrainManager"
Cohesion: 0.16
Nodes (9): JarvisBrainManager, Path, Ensure JARVIS_MEMORY.md exists on disk., Build lean, structured planning prompt with on-demand semantic memory., Central orchestrator for Jarvis's persistent Antigravity Brain., Synthesize a complete Master Task Specification prompt for autonomous workers., Antigravity module for Jarvis core brain and memory., test_brain_manager_evaluate_completion() (+1 more)
...
Cohesion: 0.06
Nodes (53): Path, Resolve a user-provided path string to an absolute Path. - Rejects empty…, resolve_path(), ensure_exists(), ensure_is_dir(), is_protected(), Path, Return True if the path is within a protected location. (+45 more)

### Community 25 - "ToolRegistry"
Cohesion: 0.11
Nodes (9): AgentFactory, # NOTE: params must match each tool's ``input_schema`` exactly — the plan, Registry that holds tool classes keyed by their name. Usage: registry =…, ToolRegistry, Return the tool instance if it is in the allowed set, else raise., Return the set of permitted tool names., A read‑only view of the global ``ToolRegistry`` that only exposes a whitelisted…, ScopedToolRegistry (+1 more)

### Community 26 - "test_agent_fork.py"
Cohesion: 0.24
Nodes (11): get_engine(), Used by the agent route to look up engines after delegation., isolated_sessions(), asyncio, fixture, test_adapter_accepts_bare_fork_step(), test_execute_plan_intercepts_fork_before_engine(), test_fork_task_creates_queryable_worker() (+3 more)
...
### Community 28 - "get_client"
Cohesion: 0.06
Nodes (28): get_client(), Validation helpers for WhatsApp tools., Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_group_action(), PhotoBackupPipeline, Any, Download photos from a WhatsApp chat and store them in YYYY/MM folders. (+20 more)
...
Cohesion: 0.16
Nodes (13): BaseModel, Ensure default allowed_tools is never None., Serialize to JSON for transport (parent ↔ worker session init)., The task contract defines what a worker is expected to do, with strict…, TaskContract, Create the on‑disk worker session from a contract., ActiveStreamGuardian, Active Stream Guardian for Worker Supervision. Monitors real-time worker… (+5 more)
...
Cohesion: 0.09
Nodes (38): BulkRenameTool, CopyTool, CreateFileTool, CreateFolderTool, DeleteFolderTool, ExistsTool, Any, MoveTool (+30 more)
...
### Community 42 - "AntigravityWorkerAgent"
Cohesion: 0.16
Nodes (13): Result of a single `agy run` invocation., RunResult, AntigravityWorkerAgent, Queue parent/manager guidance for the next turn., Autonomous agent driver powered by Antigravity CLI (`agy`) with 3-Job…, Set or update the approved implementation plan and advance to execution., Receive plan rejection with feedback and reset to planning phase., asyncio (+5 more)

### Community 43 - ".analyze_prompt"
Cohesion: 0.19
...
Cohesion: 0.15
Nodes (16): test_unknown_worker_still_404s(), client(), fixture, test_events_feed_and_digest_routes(), asyncio, Regression tests for tool fixes and API routes., test_adapter_accepts_valid_plan_steps(), test_adapter_rejects_bad_plan_steps() (+8 more)
...
Cohesion: 0.06
Nodes (51): _fork_task(), Fork a background worker session via the workers module and record a tracked…, launch_worker(), Create + start a worker engine and register it for polling/SSE. Shared by the…, build_execution_prompt(), build_master_task_prompt(), build_planning_prompt(), build_prompt() (+43 more)
...
Cohesion: 0.19
Nodes (15): ExecutionEngine, Core engine that validates input, executes a tool, and returns a ToolResult.…, ListRecentEmailsPipeline, Any, Pipeline that runs the ListRecentEmailsSkill. Demonstrates how higher‑level…, asyncio, test_list_drive_files_mock_mode(), test_list_drive_files_no_creds() (+7 more)

### Community 67 - "test_antigravity_brain.py"
Cohesion: 0.28
...
Cohesion: 0.36
Nodes (6): ListRecentEmailsSkill, List recent emails matching a query via Gmail API. Parameters ---------- query:…, asyncio, fixture, setup_skill(), test_list_recent_emails_pipeline_no_creds()
...
### Community 72 - "avatar-studio/page.js"
Cohesion: 0.10
Nodes (33): AvatarStudioPage(), handleIntensityChange(), pollVolume(), runChoreography(), stopMotion(), triggerEmotion(), BASIC_VRM_EMOTIONS, CHARM_EMOTIONS (+25 more)
...
### Community 85 - ".append_scratchpad"
Cohesion: 0.20
Nodes (5): Read the full content of JARVIS_MEMORY.md., Append an entry to the scratchpad section in JARVIS_MEMORY.md. Deduplicates…, Evaluate worker execution results against task criteria and produce an…, Extract durable facts from a turn via the agy daemon and persist them. Sends a…, Append historical worker receipt to MEMORY_ARCHIVE.md on disk.
...
Cohesion: 0.05
Nodes (27): _get_creds(), _load_token_path(), Any, Credentials, Path, Return a path to a ``token.json`` if one exists next to the tool or at the…, OpenCodeAdapter, Any (+19 more)
...
### Community 94 - "test_gmail_tools.py"
Cohesion: 0.33
...
Cohesion: 0.07
Nodes (35): ABC, BaseTool, Any, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, Perform the tool's operation. Returns a dictionary that will be wrapped in a…, RiskLevel (+27 more)
...
### Community 144 - "test_plan_review_and_skip_testing.py"
Cohesion: 0.32
Nodes (7): asyncio, When a plan is rejected with feedback, the worker should re-enter planning…, When plan is approved with edited markdown, it must be written to…, When skip_testing is True, worker finishes after Job 2 without entering Job 3., test_plan_approval_writes_markdown_to_fs_scope(), test_plan_rejection_routes_back_to_planning(), test_skip_testing_bypasses_job_3()
...
Cohesion: 0.11
Nodes (27): _agent_factory_for(), factory(), approve_worker_plan(), ApprovePlanRequest, cancel_worker(), fork_worker(), ForkRequest, intervene_worker() (+19 more)

### Community 177 - "main.py"
Cohesion: 0.17
Nodes (12): jarvis_exception_handler(), Request, create_app(), FastAPI, client(), fixture, asyncio, test_frontend_backend_integration_flow() (+4 more)
...
- **Why does `Repository` connect `Repository` to `agent.py`, `GmailInboundListener`, `EventType`, `._persist_to_db`, `.build_brain_context_prompt`, `tasks.py`, `test_session_handover.py`, `get_service_memory_manager`, `DriveInboundListener`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `agent.py`, `test_whatsapp.py`, `OrganizeDownloadsPipeline`, `test_file_tools_v2.py`, `TaskOrchestrator`, `execution_engine.py`, `EventType`, `OrganizeDownloadsSkill`, `skills.py`, `tools.py`, `test_tools_and_routes.py`, `WorkerEngine`, `test_search_content.py`, `BaseTool`, `ToolRegistry`, `test_gmail_tools.py`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `WorkerSession` connect `WorkerSession` to `test_opencode_worker.py`, `TaskContract`, `get`, `TaskOrchestrator`, `EventType`, `worker_events`, `_contract`, `workers.py`, `main.py`, `WorkerEngine`, `test_session_handover.py`, `_disk_sessions`, `ToolRegistry`, `test_agent_fork.py`, `clear_all_workers`?**
