...
## Corpus Check
- 216 files · ~94,358 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 10 file(s) not represented in the graph (top: (none) 5, .bat 2, .ico 1)

## Summary
- 1896 nodes · 3542 edges · 159 communities (115 shown, 27 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 255 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `42424b57`
...
## Communities (159 total, 27 thin omitted)

### Community 0 - "test_opencode_worker.py"
Cohesion: 0.14
Nodes (19): _extract_session_id(), Any, Non-interactive OpenCode CLI client. Wraps ``opencode run`` as an async…, Try to extract the OpenCode session ID from events or raw output., Result of a single ``opencode run`` invocation., Execute ``opencode run`` and collect structured results. Parameters ----------…, Execute opencode in a thread worker (optionally in a visible Windows terminal…, run_opencode() (+11 more)

### Community 1 - "test_whatsapp.py"
Cohesion: 0.05
Nodes (41): normalize_phone(), Normalize a phone number to bare digits (no +, spaces, or dashes)., Convert a phone number or bare number to a WhatsApp chat id ('<digits>@c.us')., to_chat_id(), DailyDigestPipeline, Any, Run the unread digest and send it to the given chat (run on a schedule or on…, PhotoBackupPipeline (+33 more)
...
Cohesion: 0.08
Nodes (38): cancel_worker(), clear_all_workers(), _disk_sessions(), fork_worker(), ForkRequest, get_worker(), get_worker_artifact_file(), get_worker_resolution() (+30 more)

### Community 4 - "get_client"
Cohesion: 0.06
Nodes (27): get_client(), Validation helpers for WhatsApp tools., Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_chat_action(), validate_group_action(), DownloadsNotifierPipeline, Any (+19 more)

### Community 5 - "registry/__init__.py"
Cohesion: 0.08
Nodes (35): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, OrganizeDownloadsSkill, Organize files in a download directory into subfolders by file extension.… (+27 more)
...
### Community 10 - "ensure_is_dir"
Cohesion: 0.12
Nodes (24): ensure_is_dir(), ArchiveOldFilesPipeline, Any, Zip files that have not been modified in the last N months, then remove the…, BackupDocumentsPipeline, Any, One-way incremental backup: copy files that are new or newer than the backup…, DedupeFolderPipeline (+16 more)

### Community 12 - "JarvisBrainManager"
Cohesion: 0.17
Nodes (19): get_brain_manager(), JarvisBrainManager, Path, Central orchestrator for Jarvis's persistent Antigravity Brain., Return the global JarvisBrainManager instance., Ensure JARVIS_MEMORY.md exists on disk., Antigravity module for Jarvis core brain and memory., asyncio (+11 more)

### Community 13 - "ToolRegistry"
Cohesion: 0.12
Nodes (8): # NOTE: params must match each tool's ``input_schema`` exactly — the plan, Registry that holds tool classes keyed by their name. Usage: registry =…, ToolRegistry, Return the tool instance if it is in the allowed set, else raise., Return the set of permitted tool names., A read‑only view of the global ``ToolRegistry`` that only exposes a whitelisted…, ScopedToolRegistry, TestScopedRegistry

### Community 14 - "antigravity_worker/agent/cli_client.py"
Cohesion: 0.11
Nodes (21): Jarvis Central Brain Manager powered by Antigravity CLI (`agy.exe`). Features:…, _build_args(), _extract_session_id(), Any, Non-interactive Antigravity CLI client. Wraps `agy run` (or configured CLI) as…, Execute `agy` via stream-json stdin/stdout and collect structured results., Build the argument list for official Antigravity CLI (`agy.exe`)., Try to extract the Antigravity session/conversation ID from events or raw… (+13 more)
...
Cohesion: 0.07
Nodes (38): Path, Resolve a user-provided path string to an absolute Path. - Rejects empty…, resolve_path(), ensure_exists(), ensure_is_file(), is_protected(), Path, Return True if the path is within a protected location. (+30 more)
...
Cohesion: 0.14
Nodes (16): Result of a single `agy run` invocation., RunResult, AntigravityWorkerAgent, Autonomous agent driver powered by Antigravity CLI (`agy`)., Queue parent/manager guidance for the next turn., asyncio, Unit tests for the Antigravity Autonomous Worker (Direct Execution & Streaming…, TestAntigravityCLIClient (+8 more)
...
Cohesion: 0.14
Nodes (17): test_unknown_worker_still_404s(), client(), fixture, engine(), asyncio, fixture, Regression tests for tool fixes and API routes., test_adapter_accepts_valid_plan_steps() (+9 more)
...
### Community 49 - "OpenCodeWorkerAgent"
Cohesion: 0.13
Nodes (15): _agent_factory_for(), factory(), Return an agent factory for the worker type, or None (placeholder loop)., OpenCodeWorkerAgent, Any, Autonomous agent driving OpenCode CLI execution for a background worker., Append a compact result the next decision will see., Queue guidance from the parent (user / Jarvis) for the next turn. (+7 more)
...
Cohesion: 0.12
Nodes (18): Synthesize a complete Master Task Specification prompt for autonomous workers., build_master_task_prompt(), build_prompt(), is_simple_task(), Milestone, MilestonePhase, plan_milestones(), Enum (+10 more)
...
Cohesion: 0.20
Nodes (8): ActivityFeed(), formatTimestamp(), BotMessage(), UserBubble(), API_URL, SUGGESTIONS, STATUS_STYLE, react

### Community 66 - "ExecutionEngine"
Cohesion: 0.31
Nodes (12): ExecutionEngine, Core engine that validates input, executes a tool, and returns a ToolResult.…, asyncio, test_list_drive_files_mock_mode(), test_list_drive_files_no_creds(), test_read_drive_file_missing_id(), test_read_drive_file_mock_mode(), test_search_drive_mock_mode() (+4 more)
...
### Community 81 - "get_opencode_binary"
Cohesion: 0.18
Nodes (9): _build_args(), Build the ``opencode run`` argument list., get_auto_approve(), get_opencode_binary(), get_opencode_model(), Return the path to the ``opencode`` CLI binary, or None if not found., Return the OpenCode model identifier (provider/model format), or None to use…, Return whether to run OpenCode with ``--auto`` (auto-approve permissions). (+1 more)
...
### Community 95 - "RuntimeError"
Cohesion: 0.18
Nodes (9): _get_creds(), _load_token_path(), Any, Credentials, Path, Return a path to a ``token.json`` if one exists next to the tool or at the…, Any, Execute a worker fork via the workers router / engine. (+1 more)
...
### Community 113 - "test_gmail_tools.py"
Cohesion: 0.33
...
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `agent.py`, `test_whatsapp.py`, `OrganizeDownloadsPipeline`, `test_file_tools_v2.py`, `registry/__init__.py`, `test_mutation_tools.py`, `EventType`, `JarvisEvent`, `execution_engine.py`, `ToolRegistry`, `skills.py`, `tools.py`, `test_gmail_tools.py`, `WorkerEngine`, `test_tools_and_routes.py`, `ListRecentEmailsSkill`, `test_search_content.py`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `ToolRegistry` connect `ToolRegistry` to `ExecutionEngine`, `registry/__init__.py`, `execution_engine.py`, `JarvisBrainManager`, `antigravity_worker/agent/cli_client.py`, `WorkerEngine`, `OpenCodeAdapter`, `RuntimeError`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `JarvisBrainManager` connect `JarvisBrainManager` to `agent.py`, `ToolRegistry`, `antigravity_worker/agent/cli_client.py`, `.analyze_prompt`, `antigravity_worker/agent/milestones.py`?**
