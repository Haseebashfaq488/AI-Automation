...
## Summary
- 1694 nodes · 2978 edges · 146 communities (102 shown, 28 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 185 edges (avg confidence: 0.92)
...
Cohesion: 0.08
Nodes (33): ABC, BaseTool, Enum, str, Abstract base class for all filesystem tools. Subclasses must define ``name``,…, RiskLevel, OrganizeDownloadsSkill, Organize files in a download directory into subfolders by file extension.… (+25 more)
...
Cohesion: 0.07
Nodes (24): get_client(), Return a value the sidecar can resolve: a full chat id, or a name/number string., resolve_recipient(), validate_group_action(), PhotoBackupPipeline, Any, Download photos from a WhatsApp chat and store them in YYYY/MM folders., Any (+16 more)
...
Cohesion: 0.15
Nodes (17): cancel_worker(), clear_all_workers(), ForkRequest, intervene_worker(), InterveneRequest, open_agy_terminal(), open_desktop(), open_terminal() (+9 more)
...
### Community 7 - "get"
Cohesion: 0.20
Nodes (10): get_worker_artifact_file(), get_worker_resolution(), get_worker_result(), get_worker_supervisor_status(), list_worker_artifacts(), get, Retrieve active stream guardian supervision metrics and violations., Retrieve Jarvis's post-execution evaluation and completion verdict. (+2 more)
...
Cohesion: 0.05
Nodes (43): normalize_phone(), Validation helpers for WhatsApp tools., Normalize a phone number to bare digits (no +, spaces, or dashes)., Convert a phone number or bare number to a WhatsApp chat id ('<digits>@c.us')., to_chat_id(), validate_chat_action(), DailyDigestPipeline, Any (+35 more)
...
Cohesion: 0.05
Nodes (46): JarvisBrainManager, Any, Path, Append an entry to the scratchpad section in JARVIS_MEMORY.md., Record activity timestamp to reset idle timer., Evaluate instant conversational intents (<5ms response time)., Build structured planning prompt with complete tool knowledge., Safely extract JSON object from raw response text. (+38 more)
...
Cohesion: 0.19
Nodes (16): _execute_plan(), _fork_task(), Execute plan steps sequentially and return collected results., Fork a background worker session via the workers module and record a tracked…, get_engine(), Used by the agent route to look up engines after delegation., isolated_sessions(), asyncio (+8 more)
...
Cohesion: 0.13
Nodes (17): fork_worker(), launch_worker(), Create (fork) a new worker session and start its loop in the background., Create + start a worker engine and register it for polling/SSE. Shared by the…, BaseModel, The task contract defines what a worker is expected to do, with strict…, Ensure default allowed_tools is never None., Serialize to JSON for transport (parent ↔ worker session init). (+9 more)
...
Cohesion: 0.27
Nodes (12): ValidationError, ExecutionEngine, Any, Core engine that validates input, executes a tool, and returns a ToolResult.…, asyncio, fixture, setup_registry(), test_send_email_happy_path_no_creds() (+4 more)
...
Cohesion: 0.19
Nodes (12): Result of a single `agy run` invocation., RunResult, AntigravityWorkerAgent, Autonomous agent driver powered by Antigravity CLI (`agy`)., Queue parent/manager guidance for the next turn., asyncio, Unit tests for the Antigravity Autonomous Worker (Direct Execution & Streaming…, TestAntigravityCLIClient (+4 more)
...
### Community 46 - "antigravity_worker/agent/cli_client.py"
Cohesion: 0.14
Nodes (15): lifespan(), FastAPI, setup_logging(), Jarvis Central Brain Manager powered by Antigravity CLI (`agy.exe`). Features:…, get_persistent_agy_daemon(), Non-interactive Antigravity CLI client. Wraps `agy run` (or configured CLI) as…, Return the global PersistentAgyDaemon instance., get_antigravity_model() (+7 more)
...
Cohesion: 0.22
Nodes (8): 📌 Active Knowledge & Notes, 📜 Core Operational Rules (Manager Persona), 🛠️ Integrated Capabilities & Active Tools, 🧠 JARVIS LIVING MEMORY & SYSTEM CONTEXT, 🧠 JARVIS LIVING MEMORY & SYSTEM CONTEXT, 📝 Scratchpad & Ongoing Directives, 📝 Scratchpad & Temporary Notes, 👤 User Profile & Preferences
...
### Community 53 - "engine.py"
Cohesion: 0.20
Nodes (8): emit_to_queues(), Any, Subscribe a new asyncio.Queue to receive live streaming events for session_id., Unsubscribe and remove an asyncio.Queue from session_id's active subscribers., Forward a streaming event to all active async queues subscribed to session_id…, subscribe(), unsubscribe(), Queue
...
### Community 63 - "run_antigravity_cli"
Cohesion: 0.29
Nodes (8): _extract_session_id(), Any, Execute `agy` via stream-json stdin/stdout and collect structured results., Try to extract the Antigravity session/conversation ID from events or raw…, run_antigravity_cli(), _run_subprocess_sync(), get_milestone_timeout(), Maximum seconds to wait for a single milestone execution.
...
### Community 68 - "execution_engine.py"
Cohesion: 0.32
...
### Community 77 - "get_agy_binary"
Cohesion: 0.33
Nodes (6): _build_args(), Build the argument list for official Antigravity CLI (`agy.exe`)., get_agy_binary(), get_auto_approve(), Return the path to the Antigravity CLI binary (`agy` or `antigravity`), or None., Return whether to run Antigravity CLI with `--auto`.

### Community 78 - "_disk_sessions"
Cohesion: 0.40
...
- **Why does `ExecutionEngine` connect `ExecutionEngine` to `registry/__init__.py`, `ListRecentEmailsSkill`, `ToolRegistry`, `execution_engine.py`, `test_search_content.py`, `test_whatsapp.py`, `test_tools_and_routes.py`, `WorkerEngine`, `test_mutation_tools.py`, `engine.py`, `agent.py`, `main.py`, `OrganizeDownloadsPipeline`, `test_file_tools_v2.py`, `skills.py`, `tools.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `ToolRegistry` connect `ToolRegistry` to `registry/__init__.py`, `execution_engine.py`, `ExecutionEngine`, `OpenCodeAdapter`, `antigravity_worker/agent/cli_client.py`, `WorkerEngine`, `JarvisBrainManager`, `engine.py`, `ScopedToolRegistry`, `agent.py`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `JarvisBrainManager` connect `JarvisBrainManager` to `ToolRegistry`, `antigravity_worker/agent/cli_client.py`, `agent.py`?**
