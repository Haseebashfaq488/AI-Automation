...
export function UserBubble({ content }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[80%] whitespace-pre-wrap rounded-2xl rounded-br-md bg-zinc-100 px-4 py-2.5 text-sm text-zinc-900">
        {content}
      </div>
    </div>
  );
...
export function ThinkingBubble() {
  return (
    <div className="flex justify-start">
      <div className="flex items-center gap-1 rounded-2xl rounded-bl-md bg-zinc-900 px-4 py-3">
...
export function BotMessage({ msg, onConfirm }) {
  ...
  if (msg.error && !msg.data) {
    return (
      <div className="flex justify-start">
        <div className="max-w-[80%] whitespace-pre-wrap rounded-2xl rounded-bl-md bg-red-950 px-4 py-2.5 text-sm text-red-300">
          {msg.error}
        </div>
      </div>
    );
...
export function PlanCard({ data, prompt, onConfirm, isConfirmed = false }) {
  ...
  return (
    <div className="max-w-[85%] rounded-2xl rounded-bl-md border border-zinc-800/80 bg-zinc-900/90 p-5 shadow-xl backdrop-blur-md">
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-zinc-800/60 pt-3.5">
        <div className="flex items-center gap-2">
          <button
            onClick={() => !isConfirmed && onConfirm(prompt, data.plan_id)}
            disabled={isConfirmed}
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-semibold shadow-md transition ${
              isConfirmed
    ...
    <div className="max-w-[85%] rounded-2xl rounded-bl-md border border-zinc-800/80 bg-zinc-900/90 p-5 shadow-xl backdrop-blur-md">
      ...
      <div className="mt-3.5 space-y-2.5">
        {data.results.map((r) => (
          <div
            key={r.index}
            className="rounded-xl border border-zinc-800/60 bg-zinc-950/70 p-3.5"
          >
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="rounded bg-zinc-800/70 px-1.5 py-0.5 font-mono text-[10px] text-zinc-400">
                  {r.tool}
                </span>
            ...
            {r.error && (
              <p className="mt-2 rounded-lg border border-red-900/50 bg-red-950/40 p-2 text-xs text-red-400">
